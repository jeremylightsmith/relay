...
const PICK = {
  ...
  properties: {
    ...
    task: { type: 'string', description: 'One-sentence name of the first unchecked task, or "" when all_done' },
  },
}
...
for (let cycle = 1; cycle <= MAX_CYCLES && !allDone; cycle++) {
  const picked = await agent(
    'Read `plan.md`. Find the FIRST task whose checkbox is still `- [ ]`. Do NOT implement anything — only select it. ' +
    'Set all_done=true only if every task is already `- [x]`.',
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS && !approved; attempt++) {
    const impl = await agent(
      ...
      { agentType: 'plan-implementer', schema: IMPL, phase: 'Implement', effort: 'high', label: `implement #${cycle}.${attempt}` },
    )
    ...
    const spec = await agent(
      `Task under review (from plan.md): ${picked.task}`,
      { agentType: 'spec-reviewer', schema: VERDICT, phase: 'Spec review', effort: 'high', label: `spec #${cycle}.${attempt}` },
    )
    if (!spec || !spec.pass) { feedback = (spec && spec.findings) || 'Spec review failed to return a verdict.'; continue }

    const quality = await agent(
      `Task under review (from plan.md): ${picked.task}`,
      { agentType: 'quality-reviewer', schema: VERDICT, phase: 'Quality review', effort: 'high', label: `quality #${cycle}.${attempt}` },
    )
  ...
  await agent(
    `In plan.md, change ONLY this task's checkbox from '- [ ]' to '- [x]' and commit just that one change ` +
    `with message 'Mark ${picked.task} complete'. Change nothing else.\nTask: ${picked.task}`,
...
for (let visit = 1; visit <= MAX_FINAL_VISITS && !ready; visit++) {
  const fr = await agent(
    'Review the whole branch now (visit ' + visit + ').',
    { agentType: 'final-reviewer', schema: VERDICT, phase: 'Final review', effort: 'high', label: `final-review #${visit}` },
  )
  if (!fr || fr.pass) { ready = true; break }

  await agent(
    'Blocking findings from the whole-branch review — fix ALL of them in one pass:\n' + fr.findings,
    { agentType: 'final-fixer', phase: 'Final review', effort: 'high', label: `final-fix #${visit}` },
  )
}
...
for (let visit = 1; visit <= MAX_SMOKE_VISITS; visit++) {
  smoke = await agent(
    ...
    { agentType: 'smoke-tester', schema: SMOKE, phase: 'Smoke', effort: 'high', label: `smoke #${visit}` },
  )
  if (!smoke || smoke.verdict === 'pass') break
  if (smoke.verdict === 'blocked') break // infra/setup — don't fix-loop an environment problem

  await agent(
    'The acceptance smoke found the feature broken end-to-end — fix ALL of it in one pass:\n' + (smoke.findings || ''),
    { agentType: 'final-fixer', phase: 'Smoke', effort: 'high', label: `smoke-fix #${visit}` },
  )
}
