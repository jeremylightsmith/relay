...
**status**, and the **outcome** an agent declares when it finishes a step. Learn these six
...
| **Failed** | A step gave up for good. The card stopped, with the reason on it. | Read the failure on the card, fix what caused it, and retry the run — answering can't revive a dead step. |

**Needs input**, **In review** and **Failed** are the three that actually want something from
you — those are what show up in your "needs you" rollup.
...
This page is the working vocabulary. Every board term is defined in the
[glossary](https://github.com/jeremylightsmith/relay/blob/main/docs/glossary.md), and the full
transition tables — including run status, node-job state, and exactly what each outcome does to
both the run and the card — are in the [state reference](/docs/architecture-state).
