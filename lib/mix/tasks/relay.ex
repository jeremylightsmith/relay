defmodule Mix.Tasks.Relay do
  ...
  def run(argv) do
    ...
    case dispatch(argv, json: json?) do
      {:ok, output} ->
        IO.puts(output)
  ...
  defp dispatch(["reject", ref, note], opts), do: Relay.CLI.reject(ref, note, opts)
  ...
  defp usage do
    ...
    usage: mix relay <command> [--json]
      ...
      reject REF "note"          reject with a note; routing is derived server-side
