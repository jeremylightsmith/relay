defmodule Relay.Runs.Policy do
  ...
  fresh-pull gate, and the runner-gone resume gate, so the scheduler, the run listener, and the
  ...
  @doc """
  The scheduler's `:runner_gone`-park resume gate (RLY-200): a parked run whose reason is
  `:runner_gone`, on an agent-held card that is not blocked (`:needs_input`) or dead
  ...
  def resumable?(%{status: :parked, parked_reason: :runner_gone}, %{status: card_status} = card) do
