defmodule Schemas.Flow do
  ...
  defp validate_start_edges(changeset) do
    edges = get_field(changeset, :edges) || []
    {start_edges, rest} = Enum.split_with(edges, &(&1.from == "start"))
  ...
  defp node_keys(changeset) do
    changeset |> get_field(:nodes) |> Kernel.||([]) |> Enum.map(& &1.key)
  end
end
