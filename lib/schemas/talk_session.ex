defmodule Schemas.TalkSession do
  ...
  conversation — the runner holds the actual session on its own disk, the server holds only
  ...
  `pinned_runner_name` is the machine holding that session; every later turn is offered only
  to it (`Relay.Talk.post_message/3` copies it onto the job row, exactly as an exclusive run's
  pin rides on `node_jobs.runner_name`).
  ...
  def changeset(session, attrs \\ %{}) do
    session
    |> cast(attrs, [:claude_session_id, :pinned_runner_name, :seed_summary, :seed_fields, :cleared_through_seq])
