...
void main() {
  ...
  test(
    'answer POSTs structured picks positionally, with the board slug',
    () async {
      final adapter = FakeAdapter(
        (_) async => jsonBody({
          'data': {'ref': 'RLY-42'},
      ...
      final result = await apiWith(adapter).answer(
        ref: 'RLY-42',
        boardSlug: 'relay',
        answers: [
          {'value': 'eu'},
      ...
      expect(adapter.requests.single.path, '/api/all/cards/RLY-42/answer');
      expect(adapter.requests.single.data, {
        'board': 'relay',
        'answers': [
          {'value': 'eu'},
  ...
  test('answer POSTs a flat body for a legacy string question', () async {
    final adapter = FakeAdapter(
      (_) async => jsonBody({
        'data': {'ref': 'RLY-42'},
      }),
    ...
    await apiWith(
      adapter,
    ).answer(ref: 'RLY-42', boardSlug: 'relay', text: 'eu, please');

    // No `answers` key at all: the server 400s `answers` against a string question.
    expect(adapter.requests.single.data, {
      'board': 'relay',
      'answer': 'eu, please',
    });
  ...
  // The two codes this route emits that approve/reject cannot.
  for (final (code, status) in const [
    ('not_needs_input', 422),
    ('invalid', 400),
  ]) {
    test('a $status $code maps to DecisionFailed($code)', () async {
      ...
      final result = await apiWith(
        adapter,
      ).answer(ref: 'RLY-42', boardSlug: 'relay', text: 'eu');
