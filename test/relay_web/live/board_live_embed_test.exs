defmodule RelayWeb.BoardLiveEmbedTest do
  ...
  describe "embed card-tap bridge (RLY-94)" do
    ...
    test "the tapped column rides on the `cards` wire key the native shell reads (RLY-234)",
         %{conn: conn, board: board, backlog: backlog} do
      _second = insert(:card, stage: backlog, title: "Second", position: 2, ref_number: 2)
      ...
      # Pins the exact key (`cards`) that board_screen.dart's navContextForTap reads
      # (payload['cards']). If the server renames it, this fails instead of the swipe
      # silently going inert on device.
      assert_push_event(view, "card-tap", payload)
      assert Map.has_key?(payload, :cards)
      refute Map.has_key?(payload, :column)
    end
