defmodule RelayWeb.BoardLiveSubstatesTest do
  ...
  test "amber appears on exactly needs_input and in_review cards", ctx do
    ...
    assert has_element?(view, ~s(article[data-ref="MY#{ni.ref_number}"].border-l-warning))
    assert has_element?(view, ~s(article[data-ref="MY#{ir.ref_number}"].border-l-warning))
    refute has_element?(view, ~s(article[data-ref="MY#{ready.ref_number}"].border-l-warning))
  end
  ...
  test "a ready-awaiting-human card shows no amber", ctx do
    ...
    {:ok, view, _html} = live(conn, ~p"/board/#{board.slug}")
    refute has_element?(view, ~s(article[data-ref="MY#{awaiting.ref_number}"].border-l-warning))
  end
  ...
  test "a ready card at the terminal stage renders as Done", ctx do
    ...
    {:ok, view, _html} = live(conn, ~p"/board/#{board.slug}")
    assert has_element?(view, ~s(article[data-ref="MY#{card.ref_number}"][data-done="true"]))
  end
  ...
  test "the drawer banner switches across all four states", ctx do
    ...
    for {stage, status, sel} <- [
      ...
      {:ok, view, _html} = live(conn, ~p"/board/#{board.slug}?card=MY#{card.ref_number}")
      render_async(view)
