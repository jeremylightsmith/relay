defmodule Relay.Runs.LastProgressTest do
  ...
  test "working_run_ids includes a claimed job on a live runner, excludes a queued one",
    ...
    insert(:node_job, node_execution: exec_a, state: :claimed, runner_name: "live", claimed_at: now)
    ...
    insert(:node_job, node_execution: exec_b, state: :queued, runner_name: nil, claimed_at: nil)
  ...
  test "working_run_ids counts a claimed job on a live runner, long past the stall threshold",
       %{board: board, works: works} do
    now = DateTime.truncate(DateTime.utc_now(), :second)
  ...
  test "working_run_ids excludes a claimed job on a stale runner, which does read stalled",
       %{board: board, works: works} do
    now = DateTime.truncate(DateTime.utc_now(), :second)
    old = DateTime.add(now, -600, :second)
    insert(:runner, board: board, name: "silent", interval: 30, last_heartbeat: old)
  ...
  test "every state in NodeJob.claimed_states/0 reads as working on a live runner",
       %{board: board, works: works} do
    now = DateTime.truncate(DateTime.utc_now(), :second)
    insert(:runner, board: board, name: "live", last_heartbeat: now)
