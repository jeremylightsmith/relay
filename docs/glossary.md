...
- **Blocked (dependency)** — a card with at least one **unsatisfied** blocker. A blocker is
  *satisfied* when it sits in a top-level `:complete`-category stage — its status is irrelevant.
  A dependency-blocked card is ineligible for **automatic flow dispatch** (no flow pulls it) and
  wears a quiet ghost `Blocked by N cards` chip. It is NOT the same as **needs you** (amber, a
