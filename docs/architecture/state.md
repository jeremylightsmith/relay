...
| `blocked` | Reported **only by the runner** (RE308) when the agent could not run at all — an expired login or a usage limit. **No edge is consulted.** With a `resume_at` (a usage limit whose reset the runner knows, RE267) the node is requeued — the run stays `running` — for up to `Engine.max_usage_limit_waits/0` (3) consecutive waits; otherwise it parks, and it never spends `max_retries`, counts toward the circuit breaker, or consumes `max_loops`. Agents cannot declare it and flows cannot route on it (`Schemas.NodeExecution.routable_outcomes/0`). | Unchanged while the node is requeued; on a park, set to `needs_input` with the cause as its question; the drawer offers Retry, not an answer box. |
